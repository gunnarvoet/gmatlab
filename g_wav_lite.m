function    [wave,period,scale,coi,hcbar,signif,fft_theor] = g_wav_log(datain,fig)
%
...
[wave,period,scale,coi] = wavelet(datain,1,1,.1,2,100,'MORLET');
power = wave.^2;
n=length(datain);
...
[signif,fft_theor] = wave_signif(1,1,scale,1,lag3,0.95,length(datain));
%[signif,fft_theor] = wave_signif(1.0,1,scale,1,lag3,0.95,length(datain));
...
% end
hold on;
[c,h] = contour(time,log2(period),sig95,[1 1],'k');

% contour(time,log2(period),over_matrix,[1 1],'Color','k');
